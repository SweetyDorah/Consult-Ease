...
import { Factory, TextCursorInput, CalendarDays, CalendarArrowDown, FolderGit2, Shapes, FileText, Home, User, Building2, Mail, Phone
  , IndianRupee, Banknote, File, IdCard } from 'lucide-react';
...
const EditForm = () => {
  ...
  const settlementProofRef = useRef(null);

  // Fetch existing project data when component mounts
  useEffect(() => {
    const fetchProjectData = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/get-project/' + id)
        if (!response.ok) {
        ...
        // Parse dates to format them correctly for date inputs (YYYY-MM-DD)
        ...
        let students = projectData.student_details;
        ...
        // If students is not an array or is empty, provide default
        ...
        setFormData({
          ...
          settlement_proof: null, // Files can't be pre-filled
          
          pi_name: projectData.pi_name || user?.name || '',
          pi_department: projectData.pi_department || user?.department || '',
          pi_email: projectData.pi_email || user?.email || '',
          pi_phone: projectData.pi_phone || user?.mobile || '',
          
          co_pi_name: projectData.co_pi_name || '',
          co_pi_department: projectData.co_pi_department || '',
          co_pi_email: projectData.co_pi_email || '',
          co_pi_phone: projectData.co_pi_phone || '',
          
          amount_sanctioned: projectData.amount_sanctioned || '',
          amount_received: projectData.amount_received || '',
          settlement_status: projectData.settlement_status || '',
          amount_settled: projectData.amount_settled || '',
          proof_upload: null, // Files can't be pre-filled
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      ...
      // Only require settlement proof if it's a new form or if user is changing it
      if (id && !formData.settlement_proof) {
        // For editing, don't require the file if it's not changed
      } else if (!formData.settlement_proof) {
        newErrors.settlement_proof = 'Signed Agreement is required';
        isValid = false;
      } else if (formData.settlement_proof && !formData.settlement_proof.name.endsWith('.pdf')) {
        newErrors.settlement_proof = 'Only PDF files are allowed';
        isValid = false;
      }
      ...
      if (!formData.co_pi_phone.trim()) {
        newErrors.co_pi_phone = 'Co-PI phone is required';
        isValid = false;
      ...
      // Only require proof upload if it's needed based on settlement status and it's either a new form or user is changing it
      if (formData.settlement_status !== 'pending') {
        if (id && !formData.proof_upload) {
          // For editing, don't require the file if it's not changed and settlement status needs it
        } else if (!formData.proof_upload) {
  ...
  console.log(pair[0] + ': ' + pair[1]);
    ...
    const response = await fetch(url, {
      ...
      if (response.ok) {
        alert(id ? 'Project updated successfully!' : 'Project submitted successfully!');
        navigate('/faculty-dashboard');
      } else {
        throw new Error(result.error || (id ? 'Update failed' : 'Submission failed'));
      }
    } catch (error) {
  ...
  return (
    <div className='main-container'>
      <h3 className='header-text'>{id ? 'EDIT PROJECT DETAILS' : 'ENTER PROJECT DETAILS'}</h3>
      {/* Step Indicator */}
            ...
            <div className="row">
              ...
              <div className="col">
                <FormInput 
                  ...
                  onChange={handleChange}
                  ref={settlementProofRef}
                />
                {id && <small className="header-subtext">Upload only if you need to update the existing file</small>}
